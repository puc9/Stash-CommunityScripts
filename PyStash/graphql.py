...
def setStashConfig(url: str, apiKey: str = ''):
    global __StashConfig
    __StashConfig["url"] = url
    __StashConfig["api_key"] = apiKey


def callGraphQL(query, variables=None):
    api_key = ""
    if __StashConfig.get("api_key"):
        api_key = __StashConfig["api_key"]

    if __StashConfig.get("url") is None:
        log.error("You need to set the URL in 'config.py'")
        return None

    stash_url = __StashConfig["url"] + "/graphql"
...
def getScenesByFilter(sceneFilter: dict, page: int = 1, perPage: int = 20) -> dict[str, Any] | None:
    query = """
...
def moveSceneFile(sceneFile: int | list[int], destPath: str):
    mutation = '''
    mutation MoveFiles($fileIds: [ID!]!, $destinationFolder: String) {
        moveFiles(input: { ids: $fileIds, destination_folder: $destinationFolder })
    }
    '''
    ...
    result = callGraphQL(mutation, variables)
    if result:
        return result.get('moveFiles')
...
def getStashBoxStudioByName(studioName: str) -> dict[str, Any] | None:
    ...
    result = callStashBoxGraphQL('https://stashdb.org/graphql', query, variables)
    if result:
        return result.get('findStudio')
    return None


def findStudiosByFilter(studioFilter: dict[str, Any], page=1, perPage=40):
    query = """
    query findStudiosByFilter($studioFilter: StudioFilterType!, $filter: FindFilterType) {
